...
def make_pose_image(
    ...
    image = image.reshape((h, w, 4))[:, :, :3]  # RGBA→RGB

    #data = fig.canvas.tostring_rgb()
    data = fig.canvas.buffer_rgba()  # RGBA配列を取得
    w, h = fig.canvas.get_width_height()
    c = len(data) // (w * h)
    plt.close()
...
def make_pose_image(
    ...
    for lr in ["Left", "Right"]:
        for parts in [get_leg(lr), get_arm(lr)]:
            points = {dim: [joints[name + dim] for name in parts] for dim in ["_x", "_y", "_z"]}
            if output_type in ['both', 'gt']:
                ax.plot(points["_x"], -np.array(points["_z"]), points["_y"], color="red")
    points = {dim: [joints[name + dim] for name in get_body()] for dim in ["_x", "_y", "_z"]}
    if output_type in ['both', 'gt']:
        ax.plot(points["_x"], -np.array(points["_z"]), points["_y"], color="red", label="Ground Truth")

    # Prediction
    if sub_joints is not None:
        for lr in ["Left", "Right"]:
            for parts in [get_leg(lr), get_arm(lr)]:
                points = {dim: [sub_joints[name + dim] for name in parts] for dim in ["_x", "_y", "_z"]}
                if output_type in ['both', 'prediction']:
                    ax.plot(points["_x"], -np.array(points["_z"]), points["_y"], color="blue")
        points = {dim: [sub_joints[name + dim] for name in get_body()] for dim in ["_x", "_y", "_z"]}
        if output_type in ['both', 'prediction']:
            ax.plot(points["_x"], -np.array(points["_z"]), points["_y"], color="blue", label="Predict")
        if output_type == 'both':
            plt.legend(fontsize=9)
